...
For single-client/local-only use, the client runtime can run fully offline without any server. The server model is required when you need collaboration across clients and authoritative commit/validation.
...
| [protocol/validation.md](protocol/validation.md) | Server validation, event profile and tree profile rules, model versioning |
...
| [client/tree-actions.md](client/tree-actions.md) | Tree profile interface, treePush/Delete/Update/Move, edge cases |

### JavaScript Interface

| Document | Covers |
|----------|--------|
| [javascript-interface.md](javascript-interface.md) | Small JS API contract for client and backend factories, with examples for all protocol functionality |
...
| Term | Definition |
|------|-----------|
| `client_id` | Authenticated client/device identifier. Represents the event origin client and is distinct from event ids and tree item ids. |
