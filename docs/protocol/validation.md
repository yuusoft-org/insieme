...
- Canonical profile: validate `event` envelope (`schema`, `data`, optional `meta`) and schema payloads.
- Compatibility profile: if tree actions are enabled, validate tree payloads and preserve tree semantics (see [Tree Operation Edge Cases](../client/tree-actions.md#tree-operation-edge-cases)).

## Valid Event Types

Top-level `event.type` values for sync submissions (`submit_event`, `submit_events`):

- Canonical profile: `event` (with envelope: `schema`, `data`, optional `meta`)
- Compatibility profile only: `set`, `unset`, `treePush`, `treeDelete`, `treeUpdate`, `treeMove`
...
- Canonical profile servers **MUST** reject non-`event` types.
- Compatibility profile servers **MAY** accept tree action types for legacy clients.
- In compatibility profile, negotiated `tree_policy` **MUST** be `strict` per this spec.
...
## Tree Compatibility Rule

If tree compatibility is enabled, both client and server **MUST** reject `treeMove` where the target parent is a descendant of the moved node (`validation_failed`), preventing orphaning caused by self-descendant moves.

## Tree Compatibility Policy (Dynamic Documents)

For dynamic-document apps that keep tree compatibility enabled, servers **MUST** enforce a policy-driven gate before commit:
...
4. Transition preconditions:
   ...
   - `treeDelete`: reject nonexistent `options.id` when strict compatibility is enabled.
...
Notes:
- This policy intentionally tightens legacy tree runtime behavior (silent no-op/orphan cases) for production robustness.
- Clients **SHOULD** run equivalent local prechecks for fast UX, but server validation remains authoritative.

## Model Versioning

- Server **MUST** expose `model_version` in `connected` and `sync_response` for canonical profile deployments.
