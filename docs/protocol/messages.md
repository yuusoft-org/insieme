...
- `msg_id` (string): unique message id for tracing/debug. Generated by the sender and **MUST** be unique per connection. Not used for request/response correlation — use `payload.id` for event correlation and `payload.partitions` + `since_committed_id` for sync correlation.
...
- `supported_profiles` is optional. If omitted, server **MUST** assume `[canonical]`.
- `required_profile` is optional. If present, server **MUST** either select that profile or fail the handshake.
- `required_tree_policy` is optional. If present and compatibility profile is selected, server **MUST** either satisfy it or fail the handshake.
...
Canonical profile uses `event.type=event`. Tree action types are compatibility-only.
...
`payload.events[].id` values **MUST** be unique within a single request. Duplicate ids in one batch **MUST** be rejected as `bad_request` before item processing.
...
payload:
  ...
  capabilities:                    # required: negotiated connection capabilities
    ...
    accepted_event_types:
      - event
    # when profile=compatibility, include:
    # tree_policy: strict
...
Reason semantics:
- `validation_failed`: payload/domain/policy validation failure.
- `forbidden`: authorization failure for one or more event partitions/resources.
...
payload:
  ...
  errors:                           # required when reason is validation_failed
    - field: event.payload.data.id  # path relative to submit_event payload
      message: duplicate id
  status_updated_at: 1738451205000
```

Error field path convention:
- `errors[].field` uses dot-path notation relative to the submitted event wrapper (`submit_event.payload` item shape). Example: `event.payload.data.id`.
- In `event_rejected`, `errors` **MUST** be present when `reason=validation_failed`.
- In `event_rejected` with `reason=forbidden`, `errors` is optional and **SHOULD** identify denied partitions/resources when present.
