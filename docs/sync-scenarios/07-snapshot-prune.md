...
## Preconditions
- Local snapshot exists for `P1` at `committedId=500`.
...
- Replay committed rows where `committed_id > 500` ordered by committedId.
- Apply draft overlay ordered by `(draftClock, id)`.
